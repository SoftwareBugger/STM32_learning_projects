...
// BIT BAND conversion, each bit is mapped to 4 bytes in the bit band region. addr is either 0x20000000 or 0x40000000
#define BITBAND(addr, bitnum) ((addr & 0xF0000000)+0x2000000+((addr &0xFFFFF)<<5)+(bitnum<<2)) 
// cast addr to a correct data type
#define MEM_ADDR(addr)  *((volatile unsigned long  *)(addr)) 
// last step
#define BIT_ADDR(addr, bitnum)   MEM_ADDR(BITBAND(addr, bitnum)) 

//IO register memory mapping
...
//return the bit banding mapped mem region for input/output of GPIO
// n is in range of 0 to 15, each register is 16 bits
